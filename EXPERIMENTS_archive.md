# VAE Finetuning Experiment Archive

Full historical write-ups moved out of `EXPERIMENTS.md` to keep that file lean
(2026-08-17 reorganization). This file is **append-only, sequential-reading
history** -- superseded baselines, closed ledger entries, and every open
question's full methodology/results/read-out/kill-criterion detail, in
roughly the order they were written. `EXPERIMENTS.md` keeps only the current
baseline state, established facts, and a one-line verdict per open question
that links back into this file.

Hardware/cluster-throughput content (dataloader worker tuning, the eval
parallelization fix, TCPStore rendezvous, per-cluster production
`batch_size`) moved to `infra.md` instead of here -- that's operational
knowledge for running jobs, not a modeling-question history.

Nothing below was reworded when it moved; content is verbatim from
`EXPERIMENTS.md`'s prior form, including its own internal "see above/below"
references, which now mean "elsewhere in this file" unless noted.

## Table of contents

- [Baseline evolution history](#baseline-evolution-history) -- how the
  baseline changed from the 2026-08-01 15-epoch frozen-trunk config through
  encoder unfreeze, 256x256 resolution, and epoch-budget/LR changes to its
  current (2026-08-16) state
- [Old ledger (10-run table, pre-whole-structure-unfreeze)](#ledger)
- [Capacity diagnostic + encoder head-vs-tail unfreeze sweep](#capacity-diagnostic-open-question-2----resolved-partial)
- [The bandwidth ceiling (superseded reasoning)](#the-bandwidth-ceiling)
- [Open questions 1-22, full write-ups](#open-questions-1-22-full-write-ups)
- [Open question detail sections (20-22, 10-19 revisits, 4, 8, seed floor)](#open-question-detail-sections)
- [Loss-component / per-channel-VRMSE / normalization-stats-file feature history](#loss-component-and-config-feature-history)
...
## Open questions 1-22, full write-ups

(Item 23 -- batch_size vs wall-clock -- moved to infra.md instead, it's a hardware question.)
...
## Open question detail sections

(Extended write-ups for questions whose answer needed more than the inline numbered-list entry above: 20-22, revisits of 10/8, 11, 12, 13, 14, 15, 16-18, 19, 4, 8, and the seed-noise-floor sweep that grounds the ~1.1% threshold used everywhere in this ledger.)
